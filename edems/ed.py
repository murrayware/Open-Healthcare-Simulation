...
import simpy
from .ed_seeder import EDSeeder
...
from .dispatch import EDDispatchMixin
from .ed_treatment import EDTreatmentMixin
from .disposition import EDDispositionMixin
...
class SingleSiteSim(
    ...
    def __init__(self, cfg, external_env=None, external_eventlog=None):
        ...
        self.env.process(self._acute_queue_refresher())
        # --- ACUTE areas ---
        ...
        self._ft_name: str = (
            getattr(ft_cfg, "name", "FAST") if self._ft_enabled else "FAST"
        )
        ...
        # inside SingleSiteSim.__init__ (or your boot method)
        self._download_cap = int(getattr(self.cfg.ems, "download_capacity", 0))
        self._download_busy = 0
        self._download_patients = {}
        self._start_download_acuity_watchdog()
        self.env.process(self._acute_queue_refresher())

        # start staff scheduler (you already have this)
        self._offload_tokens = simpy.Container(self.env, init=0, capacity=10**6)
        self.env.process(self._offload_staff_scheduler())

        # NEW: start the download acuity watcher
        ...
        self._offload_tokens = simpy.Container(
            self.env, init=self._offload_staff_for_hour(0), capacity=9999
        )
        self.env.process(self._offload_staff_scheduler())
        ...
        if (
            getattr(self.cfg, "ems", None)
            and getattr(self.cfg.ems, "enabled", False)
            and getattr(self.cfg.ems, "internal_generation", True)
        ):
            self.ems_arrivals = EMSArrivalPolicy(cfg)
            self.ems_arrivals.start(self.env, make_patient_cb=self._make_patient)

        # --- Start dispatchers ---
        print("Starting acute dispatcher")
        self.env.process(self._acute_bed_dispatcher())

        print("Starting fast dispatcher")
        if self._ft_enabled and self._ft_cap > 0:
            ...
            rows.append(
                {
                    ...
                    "disp_name": p.disp_name,
                    "door_to_doc": (p.treatment_start - p.arrival_time)
                    if None not in (p.treatment_start, p.arrival_time)
                    else float("nan"),
                    "doc_to_disp": (p.disposition_time - p.treatment_start)
                    if None not in (p.disposition_time, p.treatment_start)
                    else float("nan"),
                    "door_to_bed": (p.bed_start - p.arrival_time)
                    if None not in (p.bed_start, p.arrival_time)
                    else float("nan"),
                    "ctas": p.ctas,
                    "acuity": p.acuity,
                    "is_ems": p.is_ems,
                    "is_critical": p.is_critical,
                    "lwbs_min": p.lwbs_threshold,
                    "lwbs": p.lwbs,
                    ...
                    "bed_end": p.bed_end,
                    "bed_to_doc": p.treatment_start - p.bed_start
                    if None not in (p.treatment_start, p.bed_start)
                    else float("nan"),
                    ...
                    "requires_lab": p.requires_lab,
                    "disp_to_left_ed": p.los_minutes
                    - (p.disposition_time - p.arrival_time)
                    if None not in (p.disposition_time, p.arrival_time)
                    else float("nan"),
                    "lab_start": p.lab_start,
                    "lab_end": p.lab_end,
                    "lab_minutes": p.lab_minutes,
                    "lab_is_critical": p.lab_is_critical,
                    "requires_di": p.requires_di,
                    "di_modality": p.di_modality,
                    "di_start": p.di_start,
                    "di_end": p.di_end,
                    "di_minutes": p.di_minutes,
                    ...
                    "consult_units": ",".join(getattr(p, "consult_units", []))
                    if getattr(p, "consult_units", None)
                    else "",
                    # inpatient
                    ...
                    "emergency_inpatient_time": getattr(
