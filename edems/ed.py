...
class SingleSiteSim(EMSOffloadMixin, EDTreatmentMixin, PatientGenerationMixin, LwbsMixin, OrdersMixin, InpatientFlowMixin,  DoctorManager):
    ...
    def results(self):
        ...
        for p in self.patients.values():
            rows.append({
                "pid": p.id, "area": p.area, "arrival": p.arrival_time, "disp_name":p.disp_name,
                "door_to_doc": (p.treatment_start - p.arrival_time) if None not in (p.treatment_start, p.arrival_time) else float('nan'),
                "doc_to_disp": (p.disposition_time - p.treatment_start) if None not in (p.disposition_time, p.treatment_start) else float('nan'),
                "door_to_bed": (p.bed_start - p.arrival_time) if None not in (p.bed_start, p.arrival_time) else float('nan'),
                ...
                "offload_to_clear": max(0,50-p.arrival_to_offload) if None not in (p.arrival_to_offload, p.arrival_time) else float('nan'),
                "ems_total_minutes": p.ems_total_minutes,
                "download_minutes": p.download_minutes,
                "bed_start": p.bed_start, "bed_end": p.bed_end,
                "bed_to_doc": p.treatment_start - p.bed_start if None not in (p.treatment_start, p.bed_start) else float('nan'),
                ...
                "requires_lab": p.requires_lab,
                "disp_to_left_ed": p.los_minutes - (p.disposition_time - p.treatment_start) if None not in (p.disposition_time, p.treatment_start) else float('nan'),
