...
@dataclass
class LwbsConfig:
    # Minutes; use default_factory to avoid late-binding gotchas
    acute_threshold_draw: Callable[[], float] = field(
        default_factory=lambda: (lambda: 300.0)
    )
    fast_threshold_draw: Callable[[], float] = field(
        default_factory=lambda: (lambda: 120.0)
    )
...
@dataclass
class QueueConfig:
    # How strongly waiting time boosts priority (points per hour waited)
    acute_time_weight: float = 0.8
    fast_time_weight:  float = 0.35

    # Extra multiplier by CTAS for WAITING component only (not base acuity)
    # CTAS: 1..5 → e.g., 1.0, 1.0, 1.0, 1.3, 1.5 pushes CTAS 4–5 up faster.
    ctas_wait_mult: Dict[int, float] = field(default_factory=lambda: {
        1: 2.0,   # strongest climb
        2: 1.6,
        3: 1.2,
        4: 1.0,   # baseline
        5: 0.8,   # de-emphasize
    })
    download_wait_weight: float = 1.2  # extra points per hour while in download
...
class SimConfig:
    ...
    lwbs: LwbsConfig = field(default_factory=LwbsConfig)
